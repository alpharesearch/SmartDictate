...
public class Program
{
    ...
    private static async Task TranscribeAudioStream(Stream audioStream)
    {
        ...
        {
            Console.ForegroundColor = ConsoleColor.Red;
            Console.WriteLine("ERROR: WhisperProcessor not initialized.");
            Console.ResetColor();
            audioStream.Dispose();
            return;
        }
        ...
        try
        {
            ...
            {
                currentSessionTranscribedText.AddRange(newSegments);
            }
        }
    ...
    private static void StartNewChunk()
    {
        ...
        catch { /* ignore */ } // Disposes underlying stream if not careful
        try
        {
        ...
        try
        {
            if (WaveIn.DeviceCount == 0)
            {
                ...
                { // Allow calibration to proceed and handle no mic itself
                    Console.ForegroundColor = ConsoleColor.Red;
                    Console.WriteLine("No microphones found! Connect mic & restart, or [K] to rescan.");
                    Console.ResetColor();
                }
            }
            ...
            while (!exitRequested)
            {
                if (currentCalibrationStep != CalibrationStep.None && !isRecording)
                {
                    ...
                    {
                        Console.WriteLine("Calibration aborted by Q. Requesting exit...");
                ...
                if (Console.KeyAvailable)
                {
                    ...
                    {
                        Console.WriteLine("Q pressed. Requesting exit...");
                        exitRequested = true;
                        if (isRecording && waveSource != null)
                        {
                            ...
                            DebugLog("Q - Waiting for recording stop and final processing...");
                            await stopProcessingTcs.Task;
                            DebugLog("Q - Recording stop processing confirmed complete.");
                        }
                        // No break here, !exitRequested loop condition handles it
                    }
                    else if (key.Key == ConsoleKey.K && !isRecording)
                    {
                        SelectMicrophone();
                        deviceNumber = appSettings.SelectedMicrophoneDevice;
                    }
                    else if (key.Key == ConsoleKey.M && !isRecording)
                    {
                        ChangeModelPath();
                        PrintInstructions();
                    }
                    else if (key.Key == ConsoleKey.C && !isRecording)
                    {
                        if (WaveIn.DeviceCount == 0)
                        {
                            Console.WriteLine("Cannot calibrate: No microphones detected.");
                            continue;
                        }
                        if (appSettings.SelectedMicrophoneDevice < 0)
                        {
                            Console.WriteLine("Cannot calibrate: No valid mic selected. Use [K].");
                            continue;
                        }
                        await CalibrateThresholds(appSettings.SelectedMicrophoneDevice);
                        PrintInstructions();
                    }
                    else if (key.Key == ConsoleKey.R && !isRecording)
                    {
                        if (WaveIn.DeviceCount == 0)
                        {
                            Console.WriteLine("Cannot record: No microphones detected.");
                            continue;
                        }
                        deviceNumber = appSettings.SelectedMicrophoneDevice; // Ensure using latest from settings
                        if (deviceNumber < 0)
                        {
                            Console.WriteLine("Cannot record: No valid mic selected. Use [K].");
                            continue;
                        }
                    ...
                    else if (key.Key == ConsoleKey.S && isRecording)
                    {
                        ...
                        if (waveSource != null)
                        {
                            stopProcessingTcs = new TaskCompletionSource<bool>(TaskCreationOptions.RunContinuationsAsynchronously);
                            waveSource.StopRecording();
                            DebugLog("S - Waiting for recording stop and final processing...");
                            await stopProcessingTcs.Task;
                            DebugLog("S - Recording stop processing confirmed complete.");
                        }
                    }
                    else if (key.Key == ConsoleKey.R && isRecording)
                    {
    ...
    private static void PrintInstructions()
    {
        ...
        if (WaveIn.DeviceCount > 0)
        {
            ...
            if (currentMicDevice >= 0 && currentMicDevice < WaveIn.DeviceCount)
            {
                try
                {
                    micInfo = $"Mic: [{currentMicDevice}] {WaveIn.GetCapabilities(currentMicDevice).ProductName}";
                }
                catch { micInfo = $"Mic: [{currentMicDevice}] Error"; }
            }
            else
            {
    ...
    private static void SelectMicrophone()
    {
        if (isRecording)
        {
            Console.WriteLine("\nCannot change microphone while recording. Press 'S' to stop first.");
            return;
        }
        Console.WriteLine("\n--- Select Microphone ---");
        if (WaveIn.DeviceCount == 0)
        {
            Console.ForegroundColor = ConsoleColor.Red;
            Console.WriteLine("No microphones found!");
            Console.ResetColor();
            return;
        }

        int currentDeviceNum = appSettings.SelectedMicrophoneDevice;
        if (currentDeviceNum < 0 || currentDeviceNum >= WaveIn.DeviceCount)
            currentDeviceNum = 0;
        ...
        {
            if (WaveIn.DeviceCount > 0)
                currentMicName = WaveIn.GetCapabilities(currentDeviceNum).ProductName;
        }
        catch { }
        ...
        if (!string.IsNullOrWhiteSpace(micInput) && int.TryParse(micInput, out int parsedMicNum) && parsedMicNum >= 0 && parsedMicNum < WaveIn.DeviceCount)
        {
            ...
            {
                appSettings.SelectedMicrophoneDevice = parsedMicNum;
                selectionChanged = true;
                Console.WriteLine($"Mic set to: {WaveIn.GetCapabilities(parsedMicNum).ProductName}");
            }
            else
            {
                Console.WriteLine($"Keeping current mic: {currentMicName}");
            }
        }
        else
        {
            Console.WriteLine($"No/invalid input. Keeping current mic: {currentMicName}");
        }
        if (selectionChanged)
            SaveAppSettings();
        PrintInstructions();
    ...
    private static void ChangeModelPath()
    {
        ...
        if (!string.IsNullOrWhiteSpace(newPath) && File.Exists(newPath))
        {
            ...
            {
                currentModelFilePath = newPath;
                appSettings.ModelFilePath = newPath;
                SaveAppSettings();
            ...
            {
                Console.WriteLine("Same model path. No change.");
            }
        }
    ...
    private static async Task CalibrateThresholds(int deviceNumber)
    {
        ...
        try
        {
            ...
            Console.Write("Sampling silence in 3... ");
            ...
            {
                var ordered = calibrationSamples.OrderBy(x => x).ToList();
                typicalSilence = ordered.ElementAtOrDefault((int)(ordered.Count * 0.95));
                if (typicalSilence < 0.00001f && typicalSilence >= 0)
                    typicalSilence = 0.0001f;
                else if (typicalSilence < 0)
                    typicalSilence = 0.0001f;
            }
            else
            {
                Console.WriteLine("No silence samples. Low default used.");
                typicalSilence = 0.001f;
            }
            ...
            Console.Write("Sampling speech in 3... ");
            await Task.Delay(1000);
            Console.Write("2... ");
            await Task.Delay(1000);
            Console.Write("1... ");
            ...
            {
                var ordered = calibrationSamples.OrderBy(x => x).ToList();
                typicalSpeech = ordered.ElementAtOrDefault((int)(ordered.Count * 0.10));
                if (typicalSpeech < 0.001f && typicalSpeech >= 0)
                    typicalSpeech = 0.05f;
                else if (typicalSpeech < 0)
                    typicalSpeech = 0.05f;
            }
            else
            {
                Console.WriteLine("No speech samples. Default used.");
                typicalSpeech = 0.1f;
            }
            ...
            {
                Console.WriteLine("Warning: Speech not significantly louder. Calibration may be off.");
                calibratedEnergySilenceThreshold = typicalSilence * 2.0f;
            ...
            {
                float diff = typicalSpeech - typicalSilence;
                calibratedEnergySilenceThreshold = typicalSilence + (diff * 0.25f);
            }
    ...
    {
        if (!isRecording || !(currentCalibrationStep == CalibrationStep.SamplingSilence || currentCalibrationStep == CalibrationStep.SamplingSpeech_Recording))
            return;
    ...
    private static async void WaveSource_DataAvailable(object? sender, WaveInEventArgs e)
    {
        ...
        if (currentAudioChunkStream.Length > (waveFormatForWhisper.AverageBytesPerSecond / 2))
        {
            ...
            {
                DebugLog($"Max duration ({MAX_CHUNK_DURATION_SECONDS}s) reached.");
                process = true;
            }
            else if (silenceDetectedRecently)
            {
                DebugLog($"Silence detected, processing chunk.");
                process = true;
            }
        }
        if (!activelyProcessingChunk && process)
        {
            ...
            DebugLog($"Chunk ready. Dur: {chunkDur.TotalSeconds:F1}s, Silence: {silenceDetectedRecently}, Len: {currentAudioChunkStream.Length}");
            ...
            currentTranscriptionTask = transcription; // streamToSend disposed in TranscribeAudioStream
            try
            {
                await transcription;
            }
    ...
    private static async void WaveSource_RecordingStopped(object? sender, StoppedEventArgs e)
    {
        ...
        if (sender is WaveInEvent ws)
        {
            ...
            catch (Exception ex) { DebugLog($"Err disposing WaveInEvent: {ex.Message}"); }
        }
        ...
        if (finalFile != null && finalActiveStream != null)
        {
            ...
            catch (Exception ex) { DebugLog($"Err disposing finalFile: {ex.Message}"); } // Disposes finalActiveStream too
        }
        ...
        {
            DebugLog("No audio in FINAL chunk.");
        }
        ...
        if (wasRec)
        {
            ...
            {
                Console.WriteLine("[No speech detected in this session after filtering.]");
            }
            Console.WriteLine("----------------------------------------");
        }
        if (e.Exception != null)
        {
            ...
            Console.WriteLine($"NAudio stop exception: {e.Exception.Message}");
            Console.ResetColor();
        }

        stopProcessingTcs?.TrySetResult(true);
        DebugLog("WaveSource_RecordingStopped - Signalled stopProcessingTcs.");
        if (!exitRequested)
            PrintInstructions();
    }
}
